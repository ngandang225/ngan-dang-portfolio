...
export default function SubContact() {
  const { isSubContactOpen, updateActiveNav, activeNav, updateIsSubContactOpen } =
  ...
  return (
    <OutsideClickHandler onOutsideClick={handleClose}>
      <div
        className={`fixed top-0 right-0 bg-[#f3f3f3] p-6 pt-12 lg:pt-20 z-[100] w-5/6 sm:w-3/5 md:w-1/3 xl:w-[25%] h-screen ${
          isSubContactOpen && !isClosing ? 'sub-contact-animation' : ''
        } ${isClosing ? 'sub-contact-animation-hide' : ''}`}
      >
        <IoIosClose
          className="absolute top-2 right-2 cursor-pointer rounded-full hover:bg-gray-200"
          size={32}
          onClick={handleClose}
        />
        <div className="hidden lg:block">
          <p className="text-2xl font-bold mb-5">
            <span className="text-orange-500">D</span>ang{' '}
            <span className="text-orange-500">T</span>hi <span className="text-orange-500">T</span>
            hanh <span className="text-orange-500">N</span>gan
          </p>
        ...
        <div className="lg:hidden">
          <ul className="justify-center items-start h-full font-bold flex flex-col">
            ...
            >
              <li className="p-4 py-5 home relative w-full border-b border-orange-200">Home</li>
            </Link>
            <Link
              href={'#about-me'}
              onClick={() => handleClick('about-me')}
              className={`w-full font-bold ${
                isAboutMeActive ? 'nav-link-text-active' : 'nav-link-text-inactive'
              } `}
            >
              <li className="p-4 py-5 w-full border-b border-orange-200">About Me</li>
            </Link>
            <Link
              href={'#experience'}
              onClick={() => handleClick('experience')}
              className={`w-full font-bold ${
                isExperienceActive ? 'nav-link-text-active' : 'nav-link-text-inactive'
              } `}
            >
              <li className="p-4 py-5 w-full border-b border-orange-200">Experience</li>
            </Link>
            <Link
              href={'#skills'}
              onClick={() => handleClick('skills')}
              className={`w-full font-bold ${
                isSkillsActive ? 'nav-link-text-active' : 'nav-link-text-inactive'
              } `}
            >
              <li className="p-4 py-5 w-full border-b border-orange-200">Skills</li>
            </Link>
            <Link
              href={'#testimonial'}
              onClick={() => handleClick('testimonial')}
              className={`w-full font-bold ${
                isTestimonialActive ? 'nav-link-text-active' : 'nav-link-text-inactive'
              } `}
            >
              <li className="p-4 py-5 w-full border-b border-orange-200">Testimonial</li>
            </Link>
            <Link
              href={'#contact'}
              onClick={() => handleClick('contact')}
              className={`w-full font-bold ${
                isContactActive ? 'nav-link-text-active' : 'nav-link-text-inactive'
              } `}
            >
              <li className="p-4 py-5 w-full border-b border-orange-200">Contact</li>
            </Link>
          </ul>
        </div>
      </div>
